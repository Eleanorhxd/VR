...
class Transformer(nn.Module):
    ...
    def decode(self, memory, src_mask, tgt, tgt_mask, past=None, memory_matrix=None):
        ...
        # bi_gate = BiGate(embeddings.size(-1), embeddings.size(-1)).to(embeddings.device)
        # embeddings = bi_gate(embeddings)
...
#         gated_input = self.sigmoid(self.linear(input))
#         output = input * gated_input
#         return output
